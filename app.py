...
YILLAR = list(range(2010, 2024))     # 2010..2023
YIL_BAS = 2010
YIL_SON = 2023
N_YIL = len(YILLAR)                  # 14
...
def load_data():
    ...
    tuketim_cols = [0] + list(range(1, 1 + N_YIL))             # 0,1..14
    abone_cols   = [0] + list(range(1 + N_YIL, 1 + 2*N_YIL))   # 0,15..28

    tuketim = ilce_raw.iloc[3:14, tuketim_cols].copy()
    tuketim.columns = ["İlçe"] + [f"T{y}" for y in YILLAR]
    ...
    abone.columns = ["İlçe"] + [f"A{y}" for y in YILLAR]
    ...
    tablo2["VeriTipi"] = ["Bootstrap" if y < 2020 else "Gerçek" for y in YILLAR]
...
def compute_forecast(risk_df, abone_df):
    ...
    for ilce in abone_df["İlçe"].unique():
        a0 = abone_df[abone_df["İlçe"]==ilce][f"A{YIL_BAS}"].values[0]   # 2010
        a3 = abone_df[abone_df["İlçe"]==ilce][f"A{YIL_SON}"].values[0]   # 2023
        n_period = YIL_SON - YIL_BAS                                      # 13
    ...
    for ilce in sorted(risk_df["İlçe"].unique()):
        baz_son = risk_df[(risk_df["İlçe"]==ilce)&(risk_df["Yıl"]==YIL_SON)]["Risk_Skor"].values[0]
...
if data_loaded:
        ...
        <div style="display:flex;align-items:center;gap:14px;">
            ...
            <div>
                ...
                <div style="color:#38d1e3;font-size:0.7rem;letter-spacing:2px;text-transform:uppercase;
                            margin-top:2px;">Su Güvenliği Risk Endeksi · İzmir · {YIL_BAS}–2040</div>
            </div>
        </div>
    ...
    <div style="background:linear-gradient(90deg,rgba(155,89,182,0.08),rgba(56,209,227,0.06));
        ...
        <div style="flex:1;">
            ...
            <span style="color:#d0e8f5;font-size:0.82rem;margin-left:10px;">
                {YIL_BAS}–2019 verileri block bootstrap yöntemiyle İzmir kuraklık takvimi
                referans alınarak üretilmiştir. 2020–{YIL_SON} verileri İZSU resmi kaynağındandır.
            </span>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    if sayfa == "🏠 Ana Sayfa":
        ...
        # ── Veri hesapla
        df_son = risk_df[risk_df["Yıl"]==YIL_SON].sort_values("Risk_Skor", ascending=False)
        ...
        tahtali = tablo2[tablo2["Yıl"]==YIL_SON]["Tahtalı_Doluluk_%"].values[0]
        toplam_tuketim = int(tablo1[tablo1["Yıl"]==YIL_SON]["Tüketim_m3"].sum() / 1e6)
        kayip_oran = float(tablo2[tablo2["Yıl"]==YIL_SON]["Su_Kayıp_Oranı_%"].values[0])
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:1.5rem;">
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(56,209,227,0.2);border-radius:12px;padding:1.2rem;text-align:center;">
                <div style="color:#a8d8f0;font-size:0.7rem;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;">Toplam Tüketim {YIL_SON}</div>
            ...
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,127,14,0.3);border-radius:12px;padding:1.2rem;text-align:center;">
                <div style="color:#a8d8f0;font-size:0.7rem;letter-spacing:2px;text-transform:uppercase;margin-bottom:8px;">Su Kayıp Oranı {YIL_SON}</div>
        ...
        kartlar = [
            ...
            (k2, "🟡", "Orta Risk", f"{orta_sayi} İlçe", f"{YIL_SON} yılı", "#ff7f0e"),
            (k3, "🟢", "Düşük Risk", f"{dusuk_sayi} İlçe", f"{YIL_SON} yılı", "#2ca02c"),
            (k4, "💧", "Tahtalı Doluluk", f"%{tahtali:.1f}", f"{YIL_SON} yılı", "#38d1e3"),
        ...
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:1rem;">
            ...
            <div>
                ...
                <div style="color:#ffffff;font-size:1.1rem;font-weight:600;">
                    En Riskli 3 İlçe — {YIL_SON} Risk İbresi
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:1rem;">
            ...
            <div>
                ...
                <div style="color:#ffffff;font-size:1.1rem;font-weight:600;">
                    {YIL_SON} Yılı İlçe Risk Sıralaması & Ağırlık Dağılımı
                </div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            <div style="background:rgba(56,209,227,0.08);border:1px solid rgba(56,209,227,0.2);
                ...
                <div style="color:#a8d8f0;font-size:0.8rem;line-height:1.6;">
                    ...
                    📌 Kapsam: {YIL_BAS}–{YIL_SON} · 11 İlçe · {N_YIL} yıl<br>
        ...
        kayip_ilk = float(tablo2[tablo2["Yıl"]==YIL_BAS]["Su_Kayıp_Oranı_%"].values[0])
        kayip_son = float(tablo2[tablo2["Yıl"]==YIL_SON]["Su_Kayıp_Oranı_%"].values[0])
        ...
        <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:1.5rem;">
            ...
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(56,209,227,0.3);
                ...
                <div style="color:#a8d8f0;font-size:0.72rem;">11 ilçe · {YIL_SON} · {get_risk_label(wsri_ort)}</div>
            </div>
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(44,160,44,0.3);
                ...
                <div style="color:#a8d8f0;font-size:0.72rem;">{YIL_BAS}→{YIL_SON} · Olumlu trend</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div style="display:flex;justify-content:flex-end;gap:10px;margin-bottom:0.5rem;">
            ...
            <a href="https://twitter.com/intent/tweet?text=İzmir%20Su%20Güvenliği%20Risk%20Endeksi%20%7C%20Entropy%20ağırlıklı%20bileşik%20analiz%20%7C%20{YIL_BAS}-2040%20projeksiyonu&url=https://izmirisk.streamlit.app"
               target="_blank"
    ...
    # ════════════════════════════════
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="display:inline-block;background:rgba(56,209,227,0.1);
                ...
                <span style="color:#38d1e3;font-size:0.72rem;letter-spacing:3px;font-weight:600;">
                    EXPLORATORY DATA ANALYSIS · {YIL_BAS}–{YIL_SON}
                </span>
            </div>
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                {N_YIL} yıllık seri ({YIL_BAS}–{YIL_SON}) · Bootstrap simülasyonu ile genişletildi ·
        ...
        with tab1:
            bolum_baslik("01", "RESERVOIR STORAGE", f"Baraj Doluluk Oranları ({YIL_BAS}–{YIL_SON})")
            ...
            with col2:
                for baraj, renk in [("Tahtalı_Doluluk_%","#38d1e3"),
                    ...
                    <div style="background:rgba(255,255,255,0.05);border:1px solid {renk}44;
                        ...
                        <div style="color:{ok_renk};font-size:0.78rem;">{ok} {abs(degisim):.1f} puan ({YIL_BAS}'dan)</div>
                    </div>
                    """, unsafe_allow_html=True)

            tah_min_yil = int(tablo2.loc[tablo2["Tahtalı_Doluluk_%"].idxmin(), "Yıl"])
            insight_kutusu(
                f"Tahtalı Barajı {YIL_BAS}–{YIL_SON} arasında %{tablo2['Tahtalı_Doluluk_%'].values[0]:.1f}'den "
                ...
                f"{N_YIL} yıllık seri Mann-Kendall trend testine olanak tanıyor.",
                "#ff7f0e"
            )
        ...
        with tab2:
            ...
            insight_kutusu(
                ...
                f"{min_ilce} en düşük tüketimde. Isı haritası {N_YIL} yıl boyunca "
        ...
        with tab3:
            bolum_baslik("03", "SUPPLY–DEMAND BALANCE", f"Arz-Talep Dengesi ({YIL_BAS}–{YIL_SON})")
            ...
            insight_kutusu(
                ...
                f"{N_YIL} yıllık seri uzun vadeli arz-talep dinamiklerini görselleştiriyor. "
                f"Mor çizgi bootstrap/gerçek veri sınırını gösterir.",
                "#38d1e3"
            )

        with tab4:
            bolum_baslik("04", "WATER LOSS TREND", f"Yıllık Su Kayıp Oranı Trendi ({YIL_BAS}–{YIL_SON})")
            col1, col2 = st.columns([3,1])
            with col1:
                fig = go.Figure()
                fig.add_trace(go.Scatter(
            ...
            with col2:
                ...
                <div style="background:rgba(44,160,44,0.1);border:1px solid #2ca02c44;
                    ...
                    <div style="color:#a8d8f0;font-size:0.8rem;">{YIL_BAS} → {YIL_SON}</div>
                </div>
                <div style="background:rgba(255,255,255,0.05);border-radius:8px;
            ...
            insight_kutusu(
                f"Su kayıp oranı {YIL_BAS}'daki %{ilk_kayip:.2f}'den {YIL_SON}'te %{son_kayip:.2f}'ye geriledi. "
                f"{N_YIL} yıl boyunca toplam {azalma:.2f} puanlık iyileşme sağlandı. "
                f"Mann-Kendall analizi {N_YIL} yıllık seriyle çok daha güvenilir trend tespiti yapıyor.",
                "#2ca02c"
            )
        ...
        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:12px;">
            ...
            <div style="background:rgba(255,127,14,0.07);border:1px solid rgba(255,127,14,0.25);
                ...
                <div style="color:#ff7f0e;font-size:0.7rem;font-weight:600;letter-spacing:1px;margin-bottom:6px;">
                    {YIL_BAS}–{YIL_SON} · KAYIP AZALMASI</div>
                <div style="color:#ffffff;font-size:0.9rem;font-weight:600;margin-bottom:6px;">
                    Su kayıpları istikrarlı düştü</div>
                <div style="color:#a8d8f0;font-size:0.82rem;line-height:1.6;">
                    {N_YIL} yıl boyunca su kayıp oranı kademeli olarak azaldı. Bu, İZSU'nun
            ...
            <div style="background:rgba(56,209,227,0.07);border:1px solid rgba(56,209,227,0.25);
                ...
                <div style="color:#38d1e3;font-size:0.7rem;font-weight:600;letter-spacing:1px;margin-bottom:6px;">
                    {YIL_SON} · GAZİEMİR AYRIŞMASI</div>
                <div style="color:#ffffff;font-size:0.9rem;font-weight:600;margin-bottom:6px;">
    ...
    # ════════════════════════════════
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="display:inline-block;background:rgba(56,209,227,0.1);
                        ...
                        padding:4px 16px;margin-bottom:0.8rem;">
                <span style="color:#38d1e3;font-size:0.72rem;letter-spacing:3px;font-weight:600;">
                    WATER SECURITY RISK INDEX · WSRI · {YIL_BAS}–{YIL_SON}
                </span>
            </div>
            <div style="color:#ffffff;font-size:1.8rem;font-weight:700;margin-bottom:0.3rem;">
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                Entropy ağırlıklı bileşik skor · 4 gösterge · 0–100 ölçeği · {N_YIL} yıllık seri
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_f1:
            yil_sec = st.slider("Yıl seç:", YIL_BAS, YIL_SON, YIL_SON)
        with col_f2:
        ...
        <div style="display:flex;align-items:center;gap:12px;margin:1rem 0 0.8rem 0;">
            ...
            <div>
                ...
                <div style="color:#ffffff;font-size:1.05rem;font-weight:600;">
                    İlçe Risk Skorları & {N_YIL} Yıllık Karşılaştırma</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        skor_son = df_ilce[df_ilce["Yıl"]==YIL_SON]["Risk_Skor"].values[0]
        sinif_son = df_ilce[df_ilce["Yıl"]==YIL_SON]["Risk_Sınıf"].values[0]
        renk_son = get_risk_color(skor_son)
        cagr_val = cagr_dict.get(ilce_sec, 0) * 100

        k1, k2, k3 = st.columns(3)
        for col, baslik, deger, alt, renk in [
            (k1, f"{YIL_SON} Risk Skoru", f"{skor_son:.1f}", str(sinif_son), renk_son),
            (k2, "Risk Sınıfı", str(sinif_son), f"{YIL_SON} yılı", renk_son),
            (k3, "Abone Büyüme Hızı", f"%{cagr_val:.2f}/yıl", f"CAGR {YIL_BAS}–{YIL_SON}", "#38d1e3"),
        ]:
            with col:
                st.markdown(f"""
                <div style="background:rgba(255,255,255,0.06);
    ...
    # ════════════════════════════════
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                ...
                CAGR {N_YIL} yıllık seriden hesaplanmıştır
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            fig2.add_trace(go.Scatter(
                ...
                mode="lines+markers", name=f"Gerçek+Bootstrap ({YIL_BAS}-{YIL_SON})",
            ...
            fig2.add_vline(x=YIL_SON+0.5, line_dash="dash", line_color="rgba(255,255,255,0.3)",
            ...
            fig2.update_layout(
                ...
                xaxis=dict(gridcolor="rgba(255,255,255,0.08)",
                           tickvals=list(range(YIL_BAS, 2041, 5)),
                           tickfont=dict(color="white")),
    ...
    # ════════════════════════════════
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="display:inline-block;background:rgba(56,209,227,0.1);
                        ...
                        padding:4px 16px;margin-bottom:0.8rem;">
                <span style="color:#38d1e3;font-size:0.72rem;letter-spacing:3px;font-weight:600;">
                    SPATIAL ANALYSIS · MORAN'S I + LISA · {YIL_SON}
                </span>
            </div>
            <div style="color:#ffffff;font-size:1.8rem;font-weight:700;margin-bottom:0.3rem;">
                Mekânsal Analiz
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                Yüksek riskli ilçeler birbirine komşu mu? · Global Moran's I · LISA kümeleme · {YIL_SON}
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        r_son = risk_df[risk_df["Yıl"]==YIL_SON].set_index("İlçe")["Risk_Skor"].reindex(ilceler).values
        ...
        for col, baslik, deger, alt, renk in [
            (k1, "Global Moran's I", f"{I_glob}", f"{YIL_SON} risk skorları", "#38d1e3"),
        ...
        <div style="display:flex;align-items:center;gap:12px;margin:0.5rem 0 0.8rem 0;">
            ...
            <div>
                ...
                <div style="color:#ffffff;font-size:1.05rem;font-weight:600;">
                    Moran Scatter Plot & LISA Sınıflandırması — {YIL_SON}</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            st.markdown(f"""
            <div style="color:#38d1e3;font-size:0.7rem;letter-spacing:2px;
                        margin-bottom:0.5rem;">LISA SINIFLANDIRMASI · {YIL_SON}</div>
            """, unsafe_allow_html=True)
    ...
    # ════════════════════════════════
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="display:inline-block;background:rgba(56,209,227,0.1);
                        ...
                        padding:4px 16px;margin-bottom:0.8rem;">
                <span style="color:#38d1e3;font-size:0.72rem;letter-spacing:3px;font-weight:600;">
                    DISTRICT RECOMMENDATIONS · {YIL_SON}
                </span>
            </div>
            <div style="color:#ffffff;font-size:1.8rem;font-weight:700;margin-bottom:0.3rem;">
                İlçe Bazlı Öneriler
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                Risk sınıfına göre kişiselleştirilmiş öneri · {N_YIL} yıllık trend analizi · 2040 projeksiyonu
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        skor = df_ilce[df_ilce["Yıl"]==YIL_SON]["Risk_Skor"].values[0]
        sinif = df_ilce[df_ilce["Yıl"]==YIL_SON]["Risk_Sınıf"].values[0]
        renk = get_risk_color(skor)
        ...
        for col, baslik, deger, alt, r in [
            ...
            (k2, f"{YIL_SON} Risk Skoru", f"{skor:.1f}", str(sinif), renk),
            (k3, "2040 Baz Tahmin", f"{pred_2040['Baz'].values[0]:.1f}", "Baz senaryo", "#ff7f0e"),
            (k4, "Abone Büyüme", f"%{cagr_val:.2f}/yıl", f"CAGR {YIL_BAS}–{YIL_SON}", "#38d1e3"),
        ]:
            with col:
                st.markdown(f"""
                <div style="background:rgba(255,255,255,0.06);
        ...
        with col2:
            ...
            fig.add_trace(go.Scatter(x=hist["Yıl"],y=hist["Risk_Skor"],
                mode="lines+markers",name=f"Tarihsel ({YIL_BAS}-{YIL_SON})",
            ...
            fig.add_vline(x=YIL_SON+0.5,line_dash="dash",line_color="gray")
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="display:inline-block;background:rgba(56,209,227,0.1);
                        ...
                        padding:4px 16px;margin-bottom:0.8rem;">
                <span style="color:#38d1e3;font-size:0.72rem;letter-spacing:3px;font-weight:600;">
                    INTERACTIVE RISK MAP · İZMİR · {YIL_BAS}–2040
                </span>
            </div>
            <div style="color:#ffffff;font-size:1.8rem;font-weight:700;margin-bottom:0.3rem;">
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                İlçe üzerine gel → risk skoru, sınıf ve yıl bilgisi · Yıl ({YIL_BAS}–2040) ve senaryo seçilebilir
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with c1:
            harita_yil = st.slider("Yıl:", YIL_BAS, 2040, YIL_SON, key="harita_yil")
        with c2:
    ...
    # ════════════════════════════════
        ...
        <div style="padding:1.5rem 0 1rem 0;border-bottom:1px solid rgba(56,209,227,0.2);
            ...
            <div style="color:#a8d8f0;font-size:0.9rem;">
                Radar profil · İlçe karşılaştırma · Risk simülatörü · {N_YIL} yıllık animasyonlu seri · Veri indirme
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col_r:
            st.caption(f"Radar — Risk Bileşen Profili ({YIL_SON})")
            df_r_son = risk_df[risk_df["Yıl"]==YIL_SON]
            row = risk_df[(risk_df["İlçe"]==ilce_sec)&(risk_df["Yıl"]==YIL_SON)].iloc[0]
            def minmax_col(col):
        ...
        with col_k:
            st.caption(f"Karşılaştırma — İki İlçe ({YIL_SON})")
            ...
            row1 = risk_df[(risk_df["İlçe"]==ilce_sec)&(risk_df["Yıl"]==YIL_SON)].iloc[0]
            row2 = risk_df[(risk_df["İlçe"]==karsi_ilce)&(risk_df["Yıl"]==YIL_SON)].iloc[0]
            gostergeler = [
        ...
        <div style="display:flex;align-items:center;gap:12px;margin:1.5rem 0 0.8rem 0;">
            ...
            <div>
                ...
                <div style="color:#ffffff;font-size:1.05rem;font-weight:600;">Risk Simülatörü — Anlık Duyarlılık ({YIL_SON} bazlı)</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        df_sim = risk_df[risk_df["Yıl"]==YIL_SON].copy()
        ...
        sc3.metric(f"Gerçek {YIL_SON} Skoru", f"{gercek_skor:.1f}")
        ...
        <div style="display:flex;align-items:center;gap:12px;margin:1.5rem 0 0.8rem 0;">
            ...
            <div>
                ...
                <div style="color:#ffffff;font-size:1.05rem;font-weight:600;">Animasyonlu Risk Değişimi — {YIL_BAS}–{YIL_SON} ({N_YIL} yıl)</div>
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        for i, yil in enumerate(YILLAR):
            ...
            step = dict(method="update", label=label,
                        args=[{"visible":[j==i for j in range(N_YIL)]},
        ...
        with dl2:
            dl_yil = st.selectbox("Yıl:", ["Tüm yıllar"]+YILLAR, key="dl_yil")
        with dl3:
    ...
    # ════════════════════════════════
        ...
        with col1:
            ...
            <div style="background:rgba(255,255,255,0.04);border-radius:10px;
                ...
                <div style="color:#d0e8f5;font-size:0.88rem;line-height:1.8;">
                    📌 Kaynak: İZSU Açık Veri Portalı (2020–{YIL_SON}) + Bootstrap ({YIL_BAS}–2019)<br>
                    📌 Kapsam: 11 merkez ilçe<br>
                    📌 Dönem: {YIL_BAS} – {YIL_SON} ({N_YIL} yıl)<br>
        ...
        <div style="background:rgba(155,89,182,0.07);border:1px solid rgba(155,89,182,0.25);
            ...
            <div style="color:#c39bd3;font-size:0.85rem;line-height:1.8;">
                ...
                İZSU resmi açık verisi yalnızca 2020–{YIL_SON} dönemini kapsamaktadır (4 yıl).
                Mann-Kendall trend testi ve uzun vadeli zaman serisi analizleri için bu örneklem yetersizdir.
                Akademik geçerliliği artırmak adına {YIL_BAS}–2019 arası 10 yıllık seri,
                ...
                <b style="color:#fff;">Şeffaflık:</b> Site genelinde {YIL_BAS}–2019 verisi mor renkle (🔬 Bootstrap),
                2020–{YIL_SON} verisi yeşil renkle (✅ İZSU) işaretlenmiştir. Faculty onaylı yöntem.
            </div>
        </div>
        """, unsafe_allow_html=True)
        ...
        with col2:
            ...
            <div style="background:rgba(56,209,227,0.07);border:1px solid rgba(56,209,227,0.2);
                ...
                <div style="color:#38d1e3;font-size:0.75rem;letter-spacing:1px;margin-bottom:0.6rem;">
                    HESAPLANAN AĞIRLIKLAR ({N_YIL} YIL VERİDEN)</div>
        ...
        with col1:
            formul_kutusu(
                ...
                f"τ > 0 artan trend · τ < 0 azalan trend · p < 0.05 istatistiksel anlamlılık · n={N_YIL}"
            )
        with col2:
            formul_kutusu(
        ...
        formul_kutusu(
            f"Her ilçenin {YIL_BAS}–{YIL_SON} abone büyüme oranı (CAGR) hesaplanır ve risk skoruna uygulanır.",
            f"Risk(i,t) = Risk(i,{YIL_SON}) × (1 + CAGR<sub>i</sub> × k)<sup>t−{YIL_SON}</sup> &nbsp;→&nbsp; k ∈ {{0.5, 1.0, 1.5}}",
            f"k=0.5 İyimser · k=1.0 Baz · k=1.5 Kötümser senaryo. CAGR {N_YIL} yıllık seriden hesaplandı (sağlam tahmin). Sonuç 0–100 arasında sınırlandırıldı."
        )
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.8rem;">
            <div style="background:rgba(155,89,182,0.08);border:1px solid rgba(155,89,182,0.25);
                ...
                <div style="color:#d0e8f5;font-size:0.82rem;line-height:1.7;">
                    {YIL_BAS}–2019 verileri block bootstrap simülasyonudur. Gerçek tarihsel
        ...
        sss_listesi = [
            ("Bootstrap simülasyonu nedir, neden kullanıldı?",
             f"İZSU resmi açık verisi yalnızca 2020–{YIL_SON} dönemini kapsıyor (4 yıl). Mann-Kendall trend testi gibi istatistiksel yöntemler için bu örneklem yetersiz. Block bootstrap yöntemiyle gerçek 4 yıllık veriden faydalanarak {YIL_BAS}–2019 dönemi için sentetik ama hidrolojik olarak gerçekçi bir seri üretildi. Bu, n=4 yerine n={N_YIL} ile çalışmamızı sağlıyor — Mann-Kendall ve diğer istatistiksel testler artık çok daha güvenilir."),
            ("Bootstrap verisi gerçek mi sayılır?",
             f"Hayır, {YIL_BAS}–2019 verileri sentetiktir; gerçek İZSU ölçümleri değildir. Ancak rastgele üretilmiş değil, İzmir'in gerçek kuraklık takvimine (2014, 2017 kurak; 2010-2011, 2015 yağışlı) ve nüfus büyüme oranlarına (TÜİK %1-1.5/yıl) uyumlu olarak block bootstrap yöntemiyle üretilmiştir. Site genelinde mor renkle açıkça işaretlenmiştir."),
            ...
            (f"Mann-Kendall testi {N_YIL} yıllık veriyle artık güvenilir mi?",
             f"Evet, n={N_YIL} ile Mann-Kendall'ın istatistiksel gücü çok daha yüksektir. Önceden n=4 ile p>0.05 eşiğine ulaşmak güçtü; şimdi tau ve Sen's Slope hem yön hem büyüklük açısından çok daha sağlam çıkıyor. Yine de bootstrap kaynaklı serinin {YIL_BAS}–2019 kısmı sentetik olduğundan sonuçlar İZSU referans verisiyle doğrulanmalıdır."),
