...
<style>
    ...
    /* Sidebar tamamen gizli: sayfa geçişi floating menüden yapılır */
...
if data_loaded:
    ...
    nav_left, nav_mid, nav_right = st.columns([0.35, 6.0, 0.45])
    with nav_mid:
        aktif_etiket = etiketler[sayfa_listesi.index(st.session_state.secili_sayfa)]
        if hasattr(st, "pills"):
            secili_etiket = st.pills(
                label="Sayfa seçimi",
                options=etiketler,
                default=aktif_etiket,
                key="nav_pills",
                label_visibility="collapsed",
            )
        else:
            secili_etiket = st.radio(
                label="Sayfa seçimi",
                options=etiketler,
                index=etiketler.index(aktif_etiket),
                key="nav_radio",
                horizontal=True,
                label_visibility="collapsed",
            )
